...
inventory = [{"name": 'LIMONADA', "price": '100', "quantity": '2'},{"name": 'PANTALLA', "price": '2000', "quantity": '4'}]
...
def fuse_CSV(inventory,route):
    ...
    with open(route, 'r+', newline='') as f:
        ...
        for i in range(len(db)):
            ...
            product['name'] = db[i]['Name']
            product['price'] =  db[i]['Price']
            product['quantity'] = db[i]['Quantity']
            ...
            success.append(product['name'])
            
            fusing_data.append([inventory[i]['name'],inventory[i]['price'],inventory[i]['quantity']])
            
            products.append(product.copy())
        print(success)
        print(fusing_data)
        for i in range(len(success)-1):
                if success[i] == fusing_data[i][0]:
                    fusing_data[i][0] = 'null'
                    print(fusing_data)
                if fusing_data != 'null':
                    products.append({"name": fusing_data[i][0], "price": fusing_data[i][1], "quantity": fusing_data[i][2]})
        # print()
        # print(db)
        # print(products)        
    overwrite_csv(products, 'Modulo1_Desarrollo/semana_3/db.csv',True)    
...
#save_csv(inventory, 'Modulo1_Desarrollo/semana_3/db.csv', True)
fuse_CSV(inventory, 'Modulo1_Desarrollo/semana_3/db.csv')
